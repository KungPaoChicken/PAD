...
public class Dataset {
    ...
    Dataset(int clusterSize, int elementsSize, int variableSize, String[] variableNames, UnitRow elements) {
    ...
    public String unitNameAt(int index) {
        return rows.nameAt(index);
    }

    public double maxOfVariableAt(int index) {
        double max = rows.elementAt(0, index);
        for (int i = 1; i < elementsSize; i++) {
            max = Math.max(max, rows.elementAt(i, index));
        }
        return max;
    }

    public double minOfVariableAt(int index) {
        double min = rows.elementAt(0, index);
        for (int i = 1; i < elementsSize; i++) {
            min = Math.min(min, rows.elementAt(i, index));
        }
        return min;
    }
    ...
    public Dataset normalize() {
        ...
        for (int i = 0; i < elementsSize; i++) {
            ...
            for (int j = 0; j < variableSize; j++) {
                variables.add((rows.elementAt(i, j) - minVariables[j]) / maxMinDifference[j]);
            }
    ...
    private double calculateStandardDeviation(int variableIndex) {
        double sum = 0;
        for (int i = 0; i < elementsSize; i++) {
            sum += rows.elementAt(i, variableIndex);
        }
        double mean = sum / elementsSize;

        double totalSquaredDifference = 0;
        for (int i = 0; i < elementsSize; i++) {
            totalSquaredDifference += Math.pow(rows.elementAt(i, variableIndex) - mean, 2);
        }
    ...
    public Dataset preselect(int limit) {
        ...
        for (int i = 0; i < variableSize; i++) {
            deviations.add(calculateStandardDeviation(i));
        }
...
//        Arrays.sort(preselectedIndexes);  //DO NOT SORT THE DATASET
        ...
        for (int i = 0; i < elementsSize; i++) {
            ...
            for (int j : preselectedIndexes) {
                variables.add(rows.elementAt(i, j));
            }
